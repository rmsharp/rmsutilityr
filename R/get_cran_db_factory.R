...
#' @return data base from \url{"http://cran.us.r-project.org"}
#' @export
get_cran_db_factory <- function() {
  first <- TRUE
  today <- NULL
  db <- c(Package = "A3", Version = "1.0.0", Priority = NA, Depends = "R (>= 2.15.0), xtable, pbapply",
          ...
          NeedsCompilation = "no", File = NA, Repository = "http://cran.us.r-project.org/src/contrib")
  ...
  function(new_cran_db) {
    if (missing(new_cran_db))
      new_cran_db <- FALSE
    if (first) {
      db <<- utils::available.packages(repos="http://cran.us.r-project.org")
      first <<- FALSE
      today <<- Sys.Date()
    } else {
